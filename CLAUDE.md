...
| `tests/` | **THE ONLY TEST DIRECTORY HERE**, and since 2026-08-28 it holds BOTH halves of this service's coverage. `tests/*.js` is the in-process half — assertions about this repository's own module contracts, `npm test`, no port and no container and under a second. **`tests/vendored/` is the protocol half**: byte-identical copies of the thirteen jobs in the parent project's suite that a lone mock can satisfy, plus the wallet modules five of them verify against, driven over HTTP against a CONTAINER built from this tree by `docker-compose.yml` (a throwaway in-process copy under `--no-docker`, and under coverage). They are NOT edited here — `tests/vendored/MANIFEST.js` says where each came from and `--vendor-check` reports drift. See *Tests* below for what changed and `tests/CLAUDE.md` for the rules that are not optional there. **`tests/tools/` is not tests**: the report generator, the coverage renderer, the compose-stack helpers and the throwaway-service launcher `./local-run-tests.sh` and `./run-coverage.sh` drive — in a subdirectory precisely so that `run.js`'s discovery rule needs no exclusion list. `federation-e2e/` sat beside it until trust realms made its three-container stack unnecessary; that test is `tests/federation_sso.js` in the parent project's suite now. |
...
2. **A store becomes per realm at its declaration and nowhere else** —
   ...
   **TWO MORE WERE FOUND ON 2026-08-28 AND BOTH WERE THE SAME MISTAKE MADE TWO
   ...
   but "is everything this thing is made of". The parent project's
   `tests/sts_admin_api_operations.js` is the guard for both: it mints a token
   in a realm and looks for the claim in the default realm's.
...
are VENDORED into `tests/vendored/` — byte-identical copies, listed in
...
* **The parent is the SOURCE OF TRUTH and the copies are not edited here** —
  the same rule `common/vendored/` carries, for the same reason.
  `./local-run-tests.sh --vendor-check` reports drift and `--vendor-sync` is
  the only sanctioned way those files change. Both need the parent checkout;
  neither is part of the suite's pass or fail, because a check that needs the
  other repository cannot be what decides whether this one is green.
...
`./local-run-tests.sh` and `./run-coverage.sh` are this repository's answers to
the two scripts of those names in the parent project, and both are far shorter
than their counterparts for a reason worth knowing before porting a feature
from over there: those have to BUILD and PROVISION a stack, because the tests
they run drive a browser against all of it. `tests/tools/run-report.js` is what
they drive; `tests/report/<timestamp>/` and `coverage/` are what comes out, and
both are gitignored.

**FOUR THINGS ABOUT THEM REACH OUTSIDE `tests/CLAUDE.md`, WHICH IS WHERE THE
REST IS ARGUED.**
...
1. **THE PROTOCOL JOBS RUN BY DEFAULT AND ARE VENDORED HERE, BOTH SINCE
   ...
   tree and drives the thirteen jobs against it — in a CONTAINER since the
   ...
   **And the jobs are COPIES here rather than read out of the parent**, so a
   machine with only this repository on it runs all twenty-three. It does NOT
...
4. **THE SERVICE THE PROTOCOL JOBS DRIVE IS A CONTAINER, FROM THIS
   ...
   `--service-url`, and takes it down again; the thirteen jobs are node
...
below, less `sts_persistence_postgres.js`, which needs docker, plus five others
— so they run against this working tree with no parent checkout present. The
parent remains their source of truth; the table below describes them wherever
they are read from:
...
`WSTRUST_STS_URL` / `OID4VCI_ISSUER_URL` to locate the service. **Seven of the
eight are driven over HTTP with no browser; `sts_admin_console.js` is the
...
`sts_metadata.js` was in a checkout this repository could not count on. It is
vendored, it runs on every `./local-run-tests.sh`, and a route registered and
undescribed fails the suite here.
...
gitlink is at `d2345c3` rather than the pre-reorganisation `cae2066` this file
named for five days.
...
names a file nobody edited. The closure and the one file the next bump owes it
(`common/pq_jose.js`) are in `docs/parent-project-migration.md`.
