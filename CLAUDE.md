...
| `tests/` | **THE ONLY TEST DIRECTORY HERE** since 2026-08-26: in-process assertions about this repository's own module contracts, `npm test`, no port and no container and under a second. NOT a place to put a protocol test — see *Tests* below for the line, and `tests/CLAUDE.md` for the two rules that are not optional there. **`tests/tools/` is not tests**: the report generator, the coverage renderer and the throwaway-service launcher `./local-run-tests.sh` and `./run-coverage.sh` drive — in a subdirectory precisely so that `run.js`'s discovery rule needs no exclusion list. `federation-e2e/` sat beside it until trust realms made its three-container stack unnecessary; that test is `tests/federation_sso.js` in the parent project's suite now. |
...
./local-run-tests.sh --protocol   # AND the parent's mock-only jobs, against
                                  # a throwaway copy of THIS working tree
...
**THREE THINGS ABOUT THEM REACH OUTSIDE `tests/CLAUDE.md`, WHICH IS WHERE THE
REST IS ARGUED.**

1. **`--protocol` IS THE FIRST THING HERE THAT RUNS THE PARENT'S JOBS AGAINST
   THIS WORKING TREE**, and it is a partial answer to the gitlink problem the
   last section of this file describes: their suite drives the pinned `sts/`
   checkout, so a change made here is not covered by it until somebody bumps
   the pin. This starts a throwaway copy of this tree — nine ports of its own,
   both SPIFFE Unix sockets turned off, stopped by the pid it started — and
   runs the jobs a lone mock can satisfy against it. No docker. **It runs the
   BROWSER jobs too, and there is one** — `tests/sts_admin_console.js`, which
   became a Selenium job on 2026-08-28 and is the admin console's only
   coverage against this working tree, so leaving it out would have made
   `--protocol` quietly stop covering `/admin`. That takes the mode from about
   fifteen seconds to about a minute. This runner is SERIAL, so there is never
   a second Chrome open; `--no-browser` leaves them out and says which. It
   does NOT replace bumping the pin, because it runs on a
   developer's machine and CI over there still drives what the gitlink names.
2. **A protocol job can be AHEAD of this tree** and then fails here naming a
   feature this tree has not got. That is a fact about two checkouts, and the
   report says which side each job came from so it can be read that way rather
...
Eight tests in the parent project need only this service, and the last three of
them were written for it on 2026-08-28 rather than ported:
