...
HERE, AND THERE IS NO `tests/` DIRECTORY IN THIS REPOSITORY.** That is a decision
rather than a gap, and it was made the hard way: `tests/saml11_sso.js` was
...
**The one exception is an INTEGRATION test that needs several copies of this
service**, which is `federation-e2e/` and which could not live over there: it
brings up its own three-container stack. That is the line — a test that drives
THIS service's protocol surface goes in the parent suite; a test that has to
build a topology out of this service goes here.
