...
| `xacml/` | **The XACML 3.0 engine, and currently ONLY the engine** — no route, no console page, no store, and `server.js` does not require it. Six DOM-free modules (the model, the datatypes, the function library, the static type checker, the XML reader, the PDP) plus the VENDORED OASIS conformance suite, which is **Apache-2.0 rather than this repository's MIT** and says so in `LICENSE.md`. It stands at **454 of 455 mandatory conformance cases**; the first run scored 6 and the second 434, and `xacml/CLAUDE.md` indexes the seven defects the suite caught between those numbers, because every one of them produced an engine that ran without error and was wrong. **One model, three renderings** — XML now, the JSON Profile and ALFA later — and the rule that if `xacml_pdp.js` ever asks which syntax a policy arrived in, that separation has failed. The PAP, the PIP, the embedded PEP and the remote PEP container are phases two to five and are listed there. |
