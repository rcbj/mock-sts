...
| `xacml/` | **XACML 3.0: the engine, the policy repository, the PIP and an embedded PEP.** Ten DOM-free modules plus `xacml.js`, the only one that registers a route — four of them under `/xacml`, required at 23c. **`ou=policies` in the embedded directory IS the repository**, the way `ou=federations` is the federation register, which is what buys persistence in all three modes, per-realm isolation and `ldapsearch` for nothing. The PIP reads attributes off the subject's own entry. **THE ONLY FAMILY HERE THAT ANSWERS A QUESTION ABOUT SOMEBODY ELSE'S BOUNDARY** — every other protocol authenticates or provisions somebody, and this one is handed a subject authenticated elsewhere and asked whether they may. Held to the VENDORED OASIS conformance suite (**454 of 455 mandatory cases**), which is **Apache-2.0 rather than this repository's MIT** and says so in `LICENSE.md`. `xacml/CLAUDE.md` indexes the eleven defects those tests caught — seven in the engine, four in the wiring — because every one produced a service that ran without error and decided wrongly. **One model, three renderings**: XML and the JSON Profile now, ALFA later. The PAP, ALFA and the remote PEP container are phases three to five and are listed there. |
