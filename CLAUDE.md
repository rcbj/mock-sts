...
| `federation-e2e/` | **ONE OF THE TWO KINDS OF TEST THIS REPOSITORY HOLDS**, and the integration one. Three containers — two identity services and one web application that has never heard of federation — and one sign-in driven across all three. `./run.sh`, or `./run-host.sh` with no docker at all. |
| `tests/` | **THE OTHER KIND**, added 2026-08-25: in-process assertions about this repository's own module contracts, `npm test`, no port and no container and under a second. NOT a place to put a protocol test — see *Tests* below for the line, and `tests/CLAUDE.md` for the two rules that are not optional there. |
...
**There are TWO exceptions, and `tests/` in this repository holds the second
one.** Both are cases a test in the parent suite could not cover at all rather
than cases where it would merely be inconvenient:

* **An INTEGRATION test that needs several copies of this service** —
  `federation-e2e/`, which brings up its own three-container stack.
...
**FEDERATION IS THE EXCEPTION TO THE PARAGRAPH ABOVE: IT HAS A TEST, IT IS IN
THIS REPOSITORY, AND IT IS THE ONLY ONE HERE.** `federation-e2e/` is a
three-container stack — two instances of this service and a web application that
has never heard of federation — driven end to end with 56 assertions.
`./run.sh` builds and runs it; `./run-host.sh` runs the same test as three node
processes for anybody who cannot reach a docker daemon.
...
provider rather than from the partner, and that the directory, the counters and
the applications registry all record what actually happened. It asserts exactly
ONE refusal. **It found a real defect on its first run** — a foreign subject
...
in — which is the best argument for it existing.
